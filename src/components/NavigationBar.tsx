...
export default function NavigationBar(){
    ...
    return (
        ...
        <Link href="/">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
                <Link href="/profiles">
                <DropdownMenuItem className="w-56 h-11" >My Account</DropdownMenuItem>
                </Link>
                <Dialog >
  <DialogTrigger>
   <DropdownMenuItem className="text-red-500 flex items-center gap-2  w-56 h-11 " onSelect={(e) => e.preventDefault()}>
  ...
  <DialogContent className="w-100 h-40" >
    ...
    <DialogFooter>
        <DialogClose asChild>
            <Button className="h-10 w-20" variant="outline">Cancel</Button>
        </DialogClose>
            <Link href="/login">
            <Button className="bg-blue-600 text-white h-10 w-20 hover:bg-blue-600/80">Logout</Button>
            </Link>
    </DialogFooter>
  </DialogContent>
</Dialog>
