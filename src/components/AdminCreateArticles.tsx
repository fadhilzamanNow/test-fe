...
export default function AdminCreateArticles({
  handleChangeMode,
}: AdminCreateArticlesProps) {
  ...
  return (
    <div className="w-full h-full  bg-white  border border-gray-50 rounded-xl flex flex-col">
      <span
        className="w-full h-18 flex items-center border-b boder-b-gray-50 pl-5 font-medium"
        onClick={() => handleChangeMode("LIST")}
      >
        <ArrowLeft /> Create Articles
      </span>
      <div className="w-full min-h-[1162px] p-5 flex flex-col  gap-5">
        ...
        <div className="border border-dashed w-55 h-40 flex flex-col gap-3 text-xs !text-slate-500 justify-center items-center border-slate-300 rounded-lg relative">
          ...
          {preview && file ? (
            ...
            <Input
              type="file"
              className="w-full h-full inset-0 opacity-0 absolute"
              ref={fileInputRef}
        ...
        <div className="h-18 w-full flex justify-end items-center gap-4 ">
          <Button className="bg-white hover:bg-white/80 !text-black border border-bg-slate-200">
            Cancel
          </Button>
          <Button className="bg-slate-200 hover:bg-slate-200/80 !text-black">
            Preview
          </Button>
          <Button className="bg-blue-500 hover:bg-blue-500/80 !text-white">
            Cancel
          </Button>
        </div>
      </div>
